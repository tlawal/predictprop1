...
class PolymarketService {
  ...
  async fetchMarkets(params = {}) {
    const { fetcher, ...options } = params;
    const requestFetch = fetcher || fetch;

    console.log(`🔍 PolymarketService.fetchMarkets called with params:`, options);
    try {
      ...
      if (options.category) {
        ...
        const mappedCategory = categoryMap[options.category] || options.category;
      ...
      // }
      if (options.limit) searchParams.set('limit', options.limit);
      if (options.cursor) searchParams.set('cursor', options.cursor);
      // Note: sort and order parameters cause 422 errors, so they're removed

      // Use the markets endpoint for both regular and ticker mode to avoid 422 errors
      const endpoint = 'markets';

      // Set proper filtering and sorting parameters
      searchParams.set('limit', Math.min(options.limit || 100, 500));
      searchParams.set('offset', options.offset || 0);
      searchParams.set('closed', 'false'); // Only active markets

      // Special handling for ticker mode - just get regular active markets
      if (options.tickerMode) {
        // No special parameters for ticker mode to avoid 422 errors
      }

      // Add order parameter if provided - use correct Gamma API parameter names
      if (options.order && !options.tickerMode) {
        // Gamma API uses different parameter names
        if (options.order.includes('volume24hr')) {
          searchParams.set('order', 'volumeNum');
          searchParams.set('ascending', options.order.includes('desc') ? 'false' : 'true');
        } else if (options.order.includes('createdAt')) {
      ...
      if (options.tickerMode) {
        // For ticker mode, just return the first 40 active markets
      ...
      const finalMarkets = validMarkets.slice(0, options.limit || 50);
      ...
      const result = {
        ...
        next: responseData.pagination?.hasMore ? (options.offset || 0) + finalMarkets.length : null,
  ...
  transformMarket(market) {
    ...
    const rawMarketSlug = normalizeSlug(market.slug)
      || normalizeSlug(market.ticker)
      || normalizeSlug(market.marketSlug)
      || normalizeSlug(market.market_slug);

    const rawEventSlug = normalizeSlug(market.eventSlug)
      || normalizeSlug(market.event_slug)
      || (Array.isArray(market.events)
        ? normalizeSlug(market.events.find(event => normalizeSlug(event?.slug))?.slug)
        : null);
    ...
    return {
      ...
      slug: rawMarketSlug || rawEventSlug,
      ...
      source: 'polymarket',
      url: rawMarketSlug
        ? `https://polymarket.com/market/${rawMarketSlug}`
        : rawEventSlug
          ? `https://polymarket.com/event/${rawEventSlug}`
          : `https://polymarket.com/market/${market.ticker || market.id}`,
      ...
      isEvent: isEvent,
      icon: market.iconOptimized?.imageUrlOptimized || market.icon || market.imageOptimized?.imageUrlOptimized
    };
  }
   ...
   */
  async fetchMidpoint(tokenId, fetcher) {
    const requestFetch = fetcher || fetch;
    try {
      const response = await requestFetch(`${this.baseUrl}/markets/${tokenId}/midpoint`, {
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'PredictProp/1.0',
          ...(this.apiKey && { 'Authorization': `Bearer ${this.apiKey}` })
   ...
   */
  async fetchMultipleMidpoints(tokenIds, fetcher) {
    try {
      const promises = tokenIds.map(tokenId => this.fetchMidpoint(tokenId, fetcher));
